...
export default function NowShowing() {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api'}/movies`)
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          ...
          extractedMovies = data.movies;
        } else if (data.data && Array.isArray(data.data.movies)) {
          extractedMovies = data.data.movies;
        }
